...
resource "aws_ec2_transit_gateway_route_table_association" "this" {
  ...
  transit_gateway_route_table_id = data.aws_ec2_transit_gateway_route_table.prd.id
}
...
resource "aws_route53_vpc_association_authorization" "this_workloads_prd" {
  provider                       = aws.network
  vpc_id  = aws_vpc.this.id
  zone_id = data.aws_route53_zone.phz_prd.id
}

resource "aws_route53_zone_association" "this_workloads_prd" {
  zone_id = data.aws_route53_zone.phz_prd.id
  vpc_id  = aws_vpc.this.id
  depends_on = [ aws_route53_vpc_association_authorization.this_workloads_prd ]
}